...
#
# Modified by Nicola Compton, April 2021
# Changed 'raise StopIteration' to 'return' for python versions 3.7 and above
    ...
    def lut_sizer(self, byte, maximum = None):
        ...
        for count, idx in enumerate(range(6, -1, -2)):
            if maximum and count>=maximum:
                #raise StopIteration # use this for python versions older than 3.7
                return               # use this for python versions 3.7 or newer
...
def main():
    ...
    for i in os.listdir(options['directory']):
        name=(os.path.join(options['directory'],i))
        #name=(i)        
        ...
        file_path = os.path.join(fp.ximFileInfoObj.outputFolder, name)
