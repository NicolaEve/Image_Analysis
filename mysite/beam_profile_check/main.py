# Script to calculate symmetry and flatness of beams from the image
# and compare the profile to the image generated by chambers in water tank
...
class Edges(Image):
    ...
    def sobel_edges(self):
        ...
        #laplacian = cv2.Laplacian(self.remove_noise(), cv2.CV_64F)
...
class Profiles:
    ...
    def __init__(self, image, x_axis, y_axis):
        ...
        self.x_axis = x_axis
        self.y_axis = y_axis
        # __ sets a fixed varibale but will have to change with a function to set axis
        # or could do seperate classes with the same functions names? inherited classes
...
class Transform:
    ...
    def normalise(self, x_array, y_array):
    ...
    def field_size(self, df, profile):
        ...
        normalised_ys = self.normalise(xs, ys)
    ...
    def plot(self, inline):
        ...
        x_axis = new_xs
        y_axis = self.normalise(new_xs, new_ys)
        norm_profile = self.normalise(new_xs, profile)
        ratio = []
    ...
    def dose_matrix(self):
        ...
        inline_dose = self.normalise(new_xs, new_ys)
        norm_profile_x = self.normalise(new_xs, profile_x)

        # crossline
        new_xs, new_ys = self.interpolate(crossline_df, profile_y)
        crossline_dose = self.normalise(new_xs, new_ys)
        norm_profile_y = self.normalise(new_xs, profile_y)
